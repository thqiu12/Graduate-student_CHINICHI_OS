...
// ⚠️ 临时:角色选择直接进入,用于测试。任何 /login 访问者都能一键登录,
// 测试完务必把本文件还原为手机号+密码表单(参考 git show 182ce0a:frontend/src/pages/auth/Login.tsx)。
...
const LoginPage: React.FC = () => {
  ...
  const handleEnter = async (phone: string) => {
    setLoading(phone);
    try {
        ...
        <div style={{ width: '100%', maxWidth: 480 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {ROLES.map(role => (
              <button
                key={role.phone}
                onClick={() => handleEnter(role.phone)}
